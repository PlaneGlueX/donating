...
- Scripts 1-16 and 6b: core, join-quit, chat-extras, afk, inventory, phone, nav, shop (with heist tools), death, combat-log, pvp (passive mode, safe zones, spawn shield, no passive with loot), bounty, heists (cops plug in through its hooks), traps (they hurt, not kill), loot; bag.sk (storage, fill, glint, forfeits, selling at the base, the dropped duffel); hud.sk (the ammo bar, action-bar priority, the boss bar, heist sounds); placeholders.sk (the tab list, the heist board) started.
...
- Bot scenarios (`tools\node\node.exe bots\run.js <name>`): inventory-lock 41, join 4, wm-reload 5, join-quit 15, chat-extras 20, afk 10, phone 33, phone-map 48, pvp 10, combat-log 18, safezone 14, death 11, wm-ammo 33, shop 46, bounty 19, heists 77, hud 23, traps 55, loot 90, duffel 20 = 592 checks.
...
Next in the build order: ranks.sk, then Phase 2 (cars). Loot in the real client is PLAYTEST 76. Heist and trap open questions and numbers: PLAYTEST 66 and 72. The shop's prices and catalogue are PROPOSALs (owner questions in PLAYTEST 59) (the helmet/vest and bag-loss answers are in Death and combat).
