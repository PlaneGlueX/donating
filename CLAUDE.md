...
- `server\plugins\Skript\scripts\zz-*.sk` are LOCAL TEST HELPERS (`/zztestkit`, `/zzdump`, `/zzperm`, `/zzclear`, `/zzforget`, `/zzbal`, `/zzcfg`, `/zzcfgset`, `/zzafk`, `/zzafkstate`, `/zztip`, `/zzpassive`, `/zzhide`, `/zzshow`, `/zzdata`, `/zzphone`, `/zzcombat`, `/zzcombatend`, `/zzregion`, `/zzshield`, `/zzshieldoff`, `/zzwm`, `/zzammo`, `/zzshopreset`, `/zzfill`, `/zzshop`, `/zztag`, `/zzbounty`, `/zzbountyreset`, `/zzbountyip`, `/zzheist`, `/zzinheist`, `/zzheisttp`, `/zzcfgtime`, `/zzcfgtext`, `/zzcfgreload`, `/zzpapi`, `/zzpassivewhy`, `/zzspawn`, `/zzheistforget`, `/zzkick`, `/zzbagapply`, `/zzpack`, `/zztrap`, `/zztrapstate`, `/zztrapprobe`, `/zztrapbench`, `/zzhp`, `/zzloot`, `/zzlootcode`, `/zzbag`, `/zzbagadd`, `/zzbagclear`, `/zztool`, `/zzoffhand`, `/zzicon`, `/zzdatatext`, `/zzcleancalc`, `/zzduffels`, `/zzheistleft`, `/zzboosterleft`, `/zzconsole` (runs a command as the real console: Tebex refuses the RCON sender)). Never upload them.
...
- Placeholders from skript-placeholders must be named `prefix_identifier` (e.g. `%donating_level%`); current PlaceholderAPI rejects bare names. Install it as a plugin, not as a PAPI expansion.
...
- A heist = a WorldGuard cuboid `heist_<id>` (ids: 1-16 lowercase letters and digits) + its `/dheist` data. Staff setup, in order:
  ...
  3. `/dheist set <id> name <Name>`, and if needed `escape|cooldown <seconds>`, `pool <$>`, `level <n>` (the robber level needed), `advanced|pvp true|false` (`default` = the difficulty's value in core.sk).
...
- Hooks for later scripts are stub functions at the bottom of heists.sk (edit the one-line body):
  - `heistLevelOk` → levels.sk; `heistForfeit` → bag.sk; `heistNotify` → hud.sk
...
### Robber levels (levels.sk, built 2026-09-26)
- Built as "robber ranks" and renamed to levels the same day (owner: paid ranks show in the tab list, levels stay in the footer; two different things, two different words). Each level has a title (Pickpocket ... Kingpin).
- XP comes from selling loot at the base (bag.sk's bagSell calls levelSale last, so a level-up's title replaces the sale's): X = ⌊P / 100⌋ + 10 × H, P = what the sale paid (passive players get 75%, so their XP too), H = the heist runs in the sale not credited to this player before (data `level-haul::<h>` = the last run credited; selling one run's loot in several trips pays the +10 once). E.g. a full Duffel Bag ($6,000) from one heist: 60 + 10 = 70 XP. No XP for kills (it would pay for griefing).
- Levels (PROPOSAL, core.sk `level::<n>::name|xp|color`, `level::max`, `level::xp-per-dollars`, `level::xp-per-haul`): 0 Pickpocket 0, 1 Shoplifter 250, 2 Burglar 1,000, 3 Safecracker 3,000, 4 Heister 8,000, 5 Mastermind 20,000, 6 Kingpin 50,000 XP.
- What a level unlocks: heists (heistLevel(h): `/dheist set <id> level <n>` (`rank` still works as the old name), else the difficulty's default, PROPOSAL 0 / 1 / 2 / 3 for difficulty 1-4; heists.sk's heistLevelOk asks `levelOk`, the entry refusal names the level, /heists marks locked heists), heist tools (`tool::<t>::level`, PROPOSAL Safe Kit 1, Drill 2: the shop shows "Needs level 2 (Burglar)" and refuses), and later car models (garage.sk: `levelOk(p, n)`).
- XP is never taken away and a level never drops by itself; only staff move a level down (`/dlevel set` puts the XP at that level's start, so the two stay in step; `/dlevel reset` also clears the credited runs). Every 5 s (and on join) a player whose XP is worth a higher level than they're saved at is caught up (the thresholds were lowered), and the footer copies follow a settings reload. A level whose XP setting is missing can't be reached (fails closed). A level-up goes to that player only (owner, 2026-09-26: level-ups stay quiet): the LEVEL UP title, a sound, and each unlock in chat.
- Saved: data `level-xp`, `level`, `level-haul::<h>` (data saved under the old names `rank-xp` / `rank` moves over once on join; a heist's old `{heist::<id>::rank}` moves to `level` when heists.sk loads). Memory: `{-ph::level::<p>}`, `{-ph::levelnum::<p>}`, `{-ph::levelxp::<p>}` for the footer. TAB asks from its own thread, so no placeholder may call a function (a function being reloaded is missing for a moment: a Severe Error on TAB's thread, seen 2026-09-26); placeholders.sk copies the balance, the bounty's money texts and the heist name into memory once a second. Log: plugins\Skript\logs\levels.log.
- Players: `/level` (alias `/lvl`: level, XP, a progress bar, what the next level unlocks), `/levels` (every level). The phone's stats and the tab-list footer show it ("Level 2 Burglar · 1,240/3,000 XP"); /help mentions it. Staff: `/dlevel info|set|xp|reset <player> [value]` (`xp` only adds).
- The test heists in bots\scenarios set `level 0` for themselves (the difficulty defaults would lock the bots out); heists.js checks the default (difficulty 3: level 2).
- Local: the owner's account (Explosde) starts at level 0 like everyone. `/dlevel set Explosde 6` skips the grind for playtests.

### Paid ranks (ranks.sk, built 2026-09-26)
- Owner, 2026-09-26: "make separate monetized ranks that show next to tab list (higher ranks are higher on the tab list)". A tag before the name in the tab list, in chat and in join messages, a spot higher in the tab list, a bag skin and more bag room (Rank bags below; Monetization rules). No money, guns, bags or levels. Chat: LPC's `group-formats` give owner and the paid ranks "{prefix}&r &f{name}" ("[Legend] Name", the name white; the prefixes have no trailing space). Copy LPC's config.yml to Minehut.
- Each rank is a LuckPerms group (PROPOSAL, core.sk `paidranks` listed highest first, `paidrank::<id>::name|prefix|weight`): legend "&d[Legend]" 40, elite "&6[Elite]" 30, vipplus "&b[VIP+]" 20, vip "&a[VIP]" 10. The weight orders the groups and is also the prefix's priority, so the highest rank's tag shows and staff (owner, weight 100) beat every paid one. The owner sets the names and prices.
- TAB's sorting (plugins\TAB\config.yml, sorting-types): `GROUPS:owner,admin,mod,helper,builder,legend,elite,vipplus,vip,default`, then the biggest bounty, then A-Z. TAB reads the LuckPerms primary group (the highest weight), so a new paid rank's id must be added there too. The tab name's tag is TAB's `%condition:rankprefix%` (the LuckPerms prefix + a space).
- `/dranks setup` (staff/console) creates or updates the groups from core.sk (`lp creategroup`, `setweight`, `setdisplayname`, `meta setprefix <weight> "<prefix>"`). LuckPerms data is per server: run it once on Minehut. `/dranks give <player> <id|none>` removes every other paid group, then adds that one (one paid rank at a time), by UUID (LuckPerms only knows the names of players who joined; a store may sell to someone who hasn't), but a player who already has a higher rank keeps it (data `paid-rank` records the rank given: LuckPerms can't be asked reliably about offline players); `/dranks take <player> <id>` removes that rank only if it's the player's (Tebex's refund/chargeback command). /dranks and /dbooster need `donating.store` (the owner through `*`, and the console), never plain staff: they grant paid perks; a store (Tebex or similar, not chosen yet) would run it from the console on a purchase. `/dranks list` shows who online has which. Logged to plugins\Skript\logs\ranks.log. LuckPerms runs its commands asynchronously: a change shows a moment later.
- Players: `/ranks` (the ranks highest first with their tag, bag skin and bag bonus; yours; the store; a pointer to /level), `/store` (alias `/donate`: the Tebex store's address from core.sk `store::url`, "opening soon" while empty). `paidRank(p)` reads the player's LuckPerms groups through Vault (`groups of`).
- Rank bags (owner, 2026-09-26: "custom (but balanced) bags"): a rank gives every bag the player carries more room, capacity × (1 + b), b = `paidrank::<id>::bag-bonus` (PROPOSAL VIP 0.05, VIP+ 0.1, Elite 0.15, Legend 0.2), and its look (`paidrank::<id>::skin`: camo, arctic, gilded, neon; the art is make-item-art.js's BAGS; Neon's model glows with light_emission). A rank has its own and every lower rank's skin; `/bagskin [name|default]` picks one (data `bag-skin`). All capacity goes through bag.sk's `bagCapacity(p)` (the bag's %, room, the phone, the bag shop's lore, and death.sk's loss cap C, so a bigger bag risks more). Bags are still bought with in-game money and lost on death. A rank bought or lost shows on the bag within 5 s (every 5 seconds, bagRefresh, which only redraws on a change; its key is tier:total:skin:capacity). LuckPerms' `vault-group-use-displaynames: true` makes Vault return display names ("VIP+"), so a group matches by id or by name.
- Verified with bots (bots\run.js ranks, 11 checks): the tags in the tab list (player_info display names) and in chat; the client's order (26.3 PlayerTabOverlay sorts by list order, then team name, then the name; TAB's teams carry the order) puts Legend over VIP over a no-rank player who has the biggest bounty and comes first A-Z, and staff above all; a new rank replaces the old one; "none" removes it and the bounty decides again.

### Money boosters (boosters.sk, built 2026-09-26)
- Owner, 2026-09-26: "someone buys 1.5x money for the server for 30 mins and since they bought it for everyone they get 2x instead of 1.5x"; "$1/5min boost". While a booster runs, every loot sale pays P' = floor(P × b) for everyone and floor(P × (b + `booster::buyer-bonus`)) for its buyer (0.5; 0 would make it the same for all). P is the normal pay (passive players' 75% included); level XP and the robbery bounty stay on P. bagSell logs `pay=`, `extra=`, `booster=`.
- Delivered by Tebex: `dbooster add {username} 1.5 5 {purchaseQuantity}` (minutes × count, count 1-10,000; split into queued boosters of at most `booster::max-minutes` 240; the multiplier is rounded to 2 decimals, then must be over 1 and at most `booster::max-multiplier` 2). Every request and refusal is logged (a refused paid delivery must leave a trace), and so are staff stops and each cleared queue entry. One runs at a time, the rest queue (keys 100001, 100002, ... so they sort as text). The time counts down only while someone is online (Minehut sleeps an empty server).
- Saved, so a paid booster survives restarts and sleep: `{booster::active}` "name|uuid|b", `{booster::left}` seconds, `{booster::queue::<n>}` "name|uuid|b|seconds". The start and end are announced to everyone (the only non-heist broadcasts besides bounty claims and combat logs); the tab-list footer's last line is `%donating_booster%` ("⚡ 1.5× money for everyone · 23:10 · thanks Name", empty otherwise; copied to memory once a second). Players: `/booster`. Staff: `/dbooster add|stop|clear|info`. Log: plugins\Skript\logs\boosters.log.
- Verified with bots (bots\run.js boosters, 16 checks; the state is checked in variables.csv, not only after a reload): 1.5× for everyone and 2× for the buyer on a $1,000 sale, XP unchanged, the announcements and the footer, the queue, running out, refusals, staff only, surviving a reload.

### Tebex (the store)
- The official Tebex plugin (github.com/tebexio/Tebex-Minecraft, `tebex-bukkit-<version>.jar`; 2.4.6 was current on 2026-09-26) delivers purchases by running our console commands (`dranks give`, `dbooster add`; variables `{username}`, `{uuid}`, `{purchaseQuantity}`; initial, expiry, renewal, chargeback and refund commands). The owner creates the webstore and types `tebex secret <key>` in the console themselves (never enter or commit the key; it lands in plugins/Tebex/config.yml, which git ignores). Setup steps and the package list: MONETIZATION.md.
- Status (2026-09-26): the local server is connected to the webstore "Donating" (prices in USD); the owner makes the packages. Rank prices (picked with the owner's OK): VIP $4.99, VIP+ $9.99, Elite $19.99, Legend $34.99, one-time. The webstore had no URL yet (`zzconsole tebex info` shows "Webstore URL:" empty): put it in core.sk `store::url` once Tebex gives the store its domain. Tebex blocks automated browsers (Cloudflare), and computer use can only look at Chrome: the owner does the Tebex website. The key was typed in game, so it's in server/logs/latest.log: never share that log; on Minehut type it in the console panel.
...
## Monetization rules
- Changed 2026-09-26 (owner): paid perks may affect gameplay a little ("custom (but balanced) bags"; a money booster whose buyer gets more). The owner asked Minehut staff, who said it's OK as long as Minehut gets a private document listing every paid perk that affects gameplay: **MONETIZATION.md**. Keep it current and tell the owner to send the new version whenever a paid item changes. (Minehut's OK covers Minehut; Mojang's usage guidelines, which forbid selling a "competitive gameplay advantage", are separate: the owner took that risk knowingly.)
- Sold (all through Tebex, see MONETIZATION.md): paid ranks (a tag, tab-list order, bag skins, +5/10/15/20% bag capacity: ranks.sk) and server-wide money boosters (everyone 1.5×, the buyer 2×: boosters.sk).
- Still never sold: guns, ammo, gear, bags themselves (bags are bought with in-game money; a rank adds a percentage), heist tools, loot, in-game money, robber levels, heist access, protection from traps or cops.
...
- Tebex 2.4.6 (`tebex-bukkit-2.4.6.jar`, github.com/tebexio/Tebex-Minecraft, SHA-256 checked; downloaded 2026-09-26 with the owner's OK): the store. It loads at the next restart and does nothing until the owner types `tebex secret <key>` in the console. See Tebex (the store) and MONETIZATION.md.
...
- TAB (tab list designed 2026-09-25): header = the DONATING logo glyph + "Rob it. Run it. Keep it."; names = rank prefix (`%condition:rankprefix%`: the LuckPerms prefix + a space) + the name (green for passive players) + " AFK"; the right-hand column = the bounty; sorted staff first, then biggest bounty, then A-Z; footer = your balance and bounty, your robber level and XP to the next (2026-09-26), online/max, ping, /help /heists /level /bounty. Paid ranks (ranks.sk) sort above players without one. Its placeholders come from placeholders.sk (an empty result returns "&r": PlaceholderAPI would show the raw %placeholder% for nothing). TAB joins header/footer lines into one text, so a strikethrough line ends with &r. TAB 6 has no %maxplayers% (`%donating_max_players%`). `groups.yml` is tracked now too. The heist board (sidebar) uses a display condition (`%donating_in_heist%=yes`, listed first). Belowname and TAB's boss bar stay off (belowname is broken on 26.1 clients; Skript runs the boss bar).
...
- Text colors (owner, 2026-09-26: dark gray was hard to read): no `&8` (dark gray) for text in chat, titles or item lore; use `&7`. `&8` stays only for chest menu titles (drawn on the menu's light background), filler glass panes, a progress bar's empty part and the chat prefix's "»".
...
17. levels.sk (built 2026-09-26 as ranks.sk, renamed): XP to robber levels; unlocks heists and tools now, car models later (garage.sk: `levelOk`). See "Robber levels (levels.sk)" in Verified notes.
17b. ranks.sk (built 2026-09-26): paid ranks (LuckPerms groups) with a tag in the tab list and chat, sorted higher in the tab list, a bag skin and more bag room; /store. See "Paid ranks (ranks.sk)" in Verified notes.
17c. boosters.sk (built 2026-09-26): server-wide money boosters (Tebex). See "Money boosters (boosters.sk)".
18. placeholders.sk (started 2026-09-25 for the tab list: balance, bounty, passive, AFK, max players; bag, bag_pct, heist_loot, heist_spots, heist_haul, heist_risk for the heist board, copied into memory by loot.sk; level, level_num and level_xp, copied by levels.sk) (heists.sk answers in_heist / heist_name / the hologram lines)
...
- Scripts 1-17 and 6b: core, join-quit, chat-extras, afk, inventory, phone, nav, shop (with heist tools), death, combat-log, pvp (passive mode, safe zones, spawn shield, no passive with loot), bounty, heists (cops plug in through its hooks), traps (they hurt, not kill), loot; bag.sk (storage, fill, glint, forfeits, selling at the base, the dropped duffel); hud.sk (the ammo bar, action-bar priority, the boss bar, heist sounds); levels.sk (robber levels from selling loot; they unlock heists and tools; shown in the footer); ranks.sk (paid ranks: a tag in the tab list and chat, sorted higher, a bag skin and more bag room); boosters.sk (server-wide money boosters); placeholders.sk (the tab list, the heist board, the level).
...
- Bot scenarios (`tools\node\node.exe bots\run.js <name>`): inventory-lock 41, join 4, wm-reload 5, join-quit 15, chat-extras 20, afk 10, phone 33, phone-map 48, pvp 10, combat-log 18, safezone 14, death 11, wm-ammo 33, shop 46, bounty 19, heists 78, hud 23, traps 55, loot 90, duffel 20, levels 15, ranks 18, boosters 16 = 642 checks.
- Real 26.3 client (computer use): PLAYTEST 13-15, 29, 30 (visible parts), 33 (water), 34, 36-38, 40, 43, 49.

Waiting on the owner:
- Money numbers (PROPOSAL values in core.sk), the level numbers (XP per level, XP per $ and per heist, the level each difficulty and tool needs: PLAYTEST 90), the paid ranks' names, tags and prices, the Tebex account, secret key and packages (PLAYTEST 91, 95; MONETIZATION.md), the MOTD text, Nether/End and mob spawning on Minehut, whether to empty EssentialsX's `motd.txt` (PLAYTEST 30).
