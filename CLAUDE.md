...
- `server\plugins\Skript\scripts\zz-*.sk` are LOCAL TEST HELPERS (`/zztestkit`, `/zzdump`, `/zzperm`, `/zzclear`, `/zzforget`, `/zzbal`, `/zzcfg`, `/zzcfgset`, `/zzafk`, `/zzafkstate`, `/zztip`, `/zzpassive`, `/zzhide`, `/zzshow`, `/zzdata`, `/zzphone`, `/zzcombat`, `/zzcombatend`, `/zzregion`, `/zzshield`, `/zzshieldoff`, `/zzwm`, `/zzammo`, `/zzshopreset`, `/zzfill`, `/zzshop`, `/zztag`, `/zzbounty`, `/zzbountyreset`, `/zzbountyip`, `/zzheist`, `/zzinheist`, `/zzheisttp`, `/zzcfgtime`, `/zzcfgtext`, `/zzcfgreload`, `/zzpapi`, `/zzpassivewhy`, `/zzspawn`, `/zzheistforget`, `/zzkick`, `/zzbagapply`, `/zzpack`, `/zztrap`, `/zztrapstate`, `/zztrapprobe`, `/zztrapbench`, `/zzhp`, `/zzloot`, `/zzlootcode`, `/zzbag`, `/zzbagadd`, `/zzbagclear`, `/zztool`, `/zzoffhand`, `/zzicon`). Never upload them.
...
- Kinds, placed per heist with `/dloot` (staff) and saved with it:
  ...
  - **drill** (vault door): hold 1.5 s with a Drill to mount it (used up); it drills by itself for `time` (60 s), jams `jams` times (at k / (jams + 1) of the time; anyone clears a jam with a 2 s hold), shows a drill on the door and a text display ("DRILL 47% ▮▮▯ 0:32 · name"), trips the alarm (advanced heists), and opens the door. One drill per door; a second driller is refused before anything is used.
  - Piles point at a gate with `behind=<gate n>` and stay locked (pieces, no hitbox, no marker) until it opens. Whoever opens a safe (6 s) or drills a vault (10 s, if still inside) gets a head start ("claim") on the piles behind it; it ends early when their bag is full, they're 8 blocks away, leave or die.
...
- The clock starts at the first finished task (`heist::start-on` = "rob" now). Every pile empty while it runs: the clock drops to 1:00 (`loot::cleaned-out`, owner question; default on). `heistLootPool` returns the rolled total while a run is rolled, so death.sk's C uses what's really inside.
...
- Death: the loot leaves the bag and is recorded for part 2's dropped duffel (`{-bagdrop::<p>::loc|at|line::<tag>}`, the drop spot from traps.sk's `trapDropSpot`), logged as `death-lost` in bag.log. Part 2 (the duffel item and its pickup) waits for the owner's answer on partial pickups.
...
15. bag.sk (part 1 built 2026-09-25: storage per heist run, capacity, fill in the name, glint, forfeits, auto-sell at the base, the death record): still to build: dropped duffels and their pickup, a bag contents menu
16. hud.sk (started 2026-09-25: the XP bar shows the held gun's ammo; `hudBar(p, text, prio)` action-bar priority: combat 3, traps 2, loot 1, prompts 0): boss bar and titles still to build
17. ranks.sk: XP to robber ranks; unlocks heists, tools, car models
18. placeholders.sk (started 2026-09-25 for the tab list: balance, bounty, passive, AFK, max players; bag, bag_pct, heist_loot, heist_spots, heist_haul, heist_risk for the heist board, copied into memory by loot.sk): rank still to add (heists.sk answers in_heist / heist_name / the hologram lines)
...
- Scripts 1-14 and 6b: core, join-quit, chat-extras, afk, inventory, phone, nav, shop (with heist tools), death (without the duffel), combat-log, pvp (passive mode, safe zones, spawn shield, no passive with loot), bounty, heists (the HUD and cops plug in through its hooks), traps (they hurt, not kill), loot; bag.sk part 1 (storage, fill, glint, forfeits, selling at the base); hud.sk (the ammo bar, action-bar priority) and placeholders.sk (the tab list, the heist board) started.
- DonatingPhone plugin (see Phone plugin) with the resource pack in `pack\`.
- Bot scenarios (`tools\node\node.exe bots\run.js <name>`): inventory-lock 41, join 4, wm-reload 5, join-quit 15, chat-extras 20, afk 10, phone 33, phone-map 48, pvp 10, combat-log 18, safezone 14, death 11, wm-ammo 27, shop 45, bounty 19, heists 72, hud 13, traps 55, loot 86 = 546 checks.
...
- Loot: the 3 questions in PLAYTEST 81 (head starts, partial pieces and duffel pickups, the cleaned-out clock), the loot and tool numbers, and whether helmets and vests should get Donating art (they look like vanilla armor now).
- The two SpigotMC jars (MTVehicles 2.5.9, KoyaRobbery 1.2.2).

Next in the build order: bag.sk part 2 (the dropped duffel and its pickup, from the death record `{-bagdrop::<p>::...}`; as much as fits, unless the owner says otherwise), hud.sk (the boss bar with the escape clock and the alarm), ranks.sk, then Phase 2 (cars). Loot in the real client is PLAYTEST 76. Heist and trap open questions and numbers: PLAYTEST 66 and 72. The shop's prices and catalogue are PROPOSALs (owner questions in PLAYTEST 59) (the helmet/vest and bag-loss answers are in Death and combat).
