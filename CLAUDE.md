...
- `server\plugins\Skript\scripts\zz-*.sk` are LOCAL TEST HELPERS (`/zztestkit`, `/zzdump`, `/zzperm`, `/zzclear`, `/zzforget`, `/zzbal`, `/zzcfg`, `/zzcfgset`, `/zzafk`, `/zzafkstate`, `/zztip`, `/zzpassive`, `/zzhide`, `/zzshow`, `/zzdata`, `/zzphone`, `/zzcombat`, `/zzcombatend`, `/zzregion`, `/zzshield`, `/zzshieldoff`, `/zzwm`, `/zzammo`, `/zzshopreset`, `/zzfill`, `/zzshop`, `/zztag`, `/zzbounty`, `/zzbountyreset`, `/zzbountyip`, `/zzheist`, `/zzinheist`, `/zzheisttp`, `/zzcfgtime`, `/zzcfgtext`, `/zzcfgreload`, `/zzpapi`, `/zzpassivewhy`, `/zzspawn`, `/zzheistforget`, `/zzkick`, `/zzbagapply`, `/zzpack`, `/zztrap`, `/zztrapstate`, `/zztrapprobe`, `/zztrapbench`). Never upload them.
...
- Four kinds, placed per heist with `/dtrap` (staff) and saved with the heist; every trap kills (Skript `kill`: no attacker, so death.sk's cause is "trap": the heist's p, never a player's bounty, even if a player hit you just before) unless it's set to `effect=alarm` (advanced heists only: it calls `heistTripAlarm` and never kills).
  - **Lasers**: a thin full-bright rod (a non-persistent block display, red; light blue = alarm) between two points at any angle; static or blinking (`blink=true on=2 off=2 offset=0`, seconds). Hit test: the robber's box (0.6 wide, 1.8 tall, 1.5 sneaking, 0.6 swimming/gliding, shrunk 0.05 per side) against the beam (slab test), sampled every 0.5 blocks along the path so sprinting can't skip one. A blinking beam kills only 4 ticks after it shows (ping).
  ...
  - **Collapsing floors**: a one-layer area over a pit ≥ 2 deep, and the pit (depth 4) must be inside the heist region (else robbers fall out of the heist before they die); stepping on it cracks it (overlay + sound, everyone on it is combat-tagged "trap"), it drops after `delay` (0.5 s), whoever falls in dies, and it comes back after `restore` (10 s; 0 = until the room reset) once nobody's box crosses the tile layer (waiting on the rim doesn't block it). A 1-wide strip is safe if you keep moving; jumping across still cracks it (the jump event checks the take-off spot).
  - **Cameras**: a view cone (`angle` 30° half-angle, `range` 12, halved while sneaking) with line of sight (glass, panes and bars block it); seen for `grace` (1 s) trips the alarm (advanced heists), `effect=kill` makes it a turret (1.5 s). A yellow pointer shows where it looks; `sweep=90 period=8` makes it pan. Only the watched robber sees the "CAMERA" meter.
...
16. hud.sk (started 2026-09-25: the XP bar shows the held gun's ammo): boss bar, heist-board data for TAB, action bar and titles still to build
17. ranks.sk: XP to robber ranks; unlocks heists, tools, car models
18. placeholders.sk (started 2026-09-25 for the tab list: balance, bounty, passive, AFK, max players): rank, bag value, heist info still to add (heists.sk answers in_heist / heist_name / the hologram lines)
...
- Bot scenarios (`tools\node\node.exe bots\run.js <name>`): inventory-lock 41, join 4, wm-reload 5, join-quit 15, chat-extras 20, afk 10, phone 33, phone-map 48, pvp 10, combat-log 18, safezone 14, death 11, wm-ammo 21, shop 45, bounty 19, heists 72, hud 13, traps 49 = 448 checks.
...
Next in the build order: loot.sk (loot spots with a value range per reset and visible loot that empties; switch `heist::start-on` to "rob"), bag.sk (the duffel, the forfeit hook; it calls `addBounty` for robbery sales). Heist and trap open questions and numbers: PLAYTEST 66 and 72. The shop's prices and catalogue are PROPOSALs (owner questions in PLAYTEST 59) (the helmet/vest and bag-loss answers are in Death and combat).
