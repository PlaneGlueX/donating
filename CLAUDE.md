...
- Owner, 2026-09-26: "make separate monetized ranks that show next to tab list (higher ranks are higher on the tab list)". Cosmetic only (Monetization rules): a tag before the name in the tab list, in chat and in join messages, and a spot higher in the tab list. Nothing else: no money, guns, bags or levels. Chat: LPC's `group-formats` give owner and the paid ranks "{prefix}&r &f{name}" ("[Legend] Name", the name white; the prefixes have no trailing space). Copy LPC's config.yml to Minehut.
...
17b. ranks.sk (built 2026-09-26): paid, cosmetic ranks (LuckPerms groups) with a tag in the tab list and chat, sorted higher in the tab list. See "Paid ranks (ranks.sk)" in Verified notes.
...
- Bot scenarios (`tools\node\node.exe bots\run.js <name>`): inventory-lock 41, join 4, wm-reload 5, join-quit 15, chat-extras 20, afk 10, phone 33, phone-map 48, pvp 10, combat-log 18, safezone 14, death 11, wm-ammo 33, shop 46, bounty 19, heists 78, hud 23, traps 55, loot 90, duffel 20, levels 15, ranks 11 = 619 checks.
- Real 26.3 client (computer use): PLAYTEST 13-15, 29, 30 (visible parts), 33 (water), 34, 36-38, 40, 43, 49.

Waiting on the owner:
- Money numbers (PROPOSAL values in core.sk), the level numbers (XP per level, XP per $ and per heist, the level each difficulty and tool needs: PLAYTEST 90), the paid ranks' names, tags and prices, and a store (PLAYTEST 91), the MOTD text, Nether/End and mob spawning on Minehut, whether to empty EssentialsX's `motd.txt` (PLAYTEST 30).
