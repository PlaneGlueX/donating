...
- DonatingPhone: build `server\plugins\DonatingPhone.jar` (`tools\build-plugin.ps1`), upload it to `plugins/` and restart once: it creates `plugins/DonatingPhone/config.yml` and 16 phone maps in `world/data`. Then set `city-maps` and run `/dphone`. (The earlier `donating` datapack is gone; don't upload it.) Local test city: maps [[0, 20], [23, 24]] (2x2 scale-0 maps around spawn).
